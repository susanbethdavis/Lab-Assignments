...
At first glance from the output above, it seems that flight number 807 uniquely represents the flight from EWR to ATL by carrier DL. (Fill in the Blanks). 
...
Why or why not? Because we see instances of the flight number 807 being used for DL airlines and instances of the same flight number being used for WN carrier. 

If we know the flight number, carrier, and origin, can we know for sure what the destination will be? Yes (*Yes/No*)

Why or why not? We can know the destination because when given this information all the rows have a destination outputted, there are no NA values.  
...
gain_hr == 0: When gain hour is zero it means you gained no airtime

gain_hr< 0 : you lost airtime

gain_hr > 0 : you gained time in the air 

```{r,eval=F}
f2d<-filter(f2c, percent_gain_hr <0.1)
```
